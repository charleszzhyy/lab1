...
function renderEverything() {
  ...
  const baseFiltered = filterBySearch(allProjects, searchQuery);

  let finalVisible = baseFiltered;
  if (
    selectedIndex !== -1 &&
    sliceLabelsForCurrentView[selectedIndex] !== undefined
  ) {
    const yearPicked = sliceLabelsForCurrentView[selectedIndex];
    finalVisible = baseFiltered.filter(
      (p) => String(p.year) === String(yearPicked)
    );
    selectedIndex = 0;
  }

  if (titleEl) {
    const count = Array.isArray(finalVisible)
      ? finalVisible.length
      : 0;
  ...
  renderPieAndLegend(finalVisible);
}
