...
export function renderProjects(projectsArray, containerElement, headingLevel = 'h2') {
  ...
  if (!Array.isArray(projectsArray) || projectsArray.length === 0) {
    ...
    emptyMsg.textContent = 'No projects to display yet.';
  ...
  const safeHeading = validHeadings.includes(headingLevel) ? headingLevel : 'h2';
  ...
  projectsArray.forEach((project) => {
    ...
    const imgSrc = project.image || 'https://via.placeholder.com/300x150?text=No+Image';
    const descText = project.description || 'No description yet.';

    article.innerHTML = `
      <${safeHeading}>${titleText}</${safeHeading}>
